...
import { item, items } from "@/docs/facility/listDocs";
...
interface IListsProps {
  searched: listDocsType[];
}
export default function Lists({ searched }: IListsProps) {
  ...
  return (
    <Container id="list">
      ...
      <ListCover>
        {searched.slice(offSet, offSet + 10).map((arr, i) => (
          <ListBox
            key={i}
            onClick={() => {
              router.push(`/facility/detail/${i}`);
            }}
          >
            <TagBox>
              <Tag>{arr.tag}</Tag>
